...
if __name__ == "__main__":
    ...
    pooled_subject_results = np.zeros((num_subjects))
    ...
    for s_test in range(num_subjects):
        ...
        # All subjects except the test subject are used for training.
        s_train = list(range(num_subjects))
        s_train.pop(s_test)
        ...
        for epoch in range(0, num_epochs):
            ...
            training_loss[s_test,epoch] = train(model, training_loader, optimizer, device)
            validation_loss[s_test,epoch] = validate(model, validation_loader, device)

            scheduler.step(validation_loss[s_test,epoch])
